...
async function diagnoseApiKey() {
  ...
  try {
    ...
    if (!settings.geminiApiKey && !settings.openaiApiKey) {
      console.error(
        "No API key found. Please set your API key in extension settings first.",
      );
      return;
    }
  } catch (error) {
...
async function testGeminiKey(settings) {
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 400) {
        if (responseText.includes("api.key.invalid")) {
          ...
          console.log(
            "   Issue: API key doesn't have permission for this model",
          );
        } else if (
          responseText.includes("location") ||
          responseText.includes("geographic") ||
          responseText.includes("region")
        ) {
          console.log(
            "   Issue: GEOGRAPHIC/LOCATION RESTRICTION - Your country/region is not supported for Gemini API",
          );
          console.log(
            "   Solution: Use a VPN to connect from a supported country (US, UK, Canada, Australia, etc.)",
          );
          console.log(
            "   More info: https://cloud.google.com/generative-ai/docs/availability",
          );
        }
    ...
    console.log(
      "   Status: Network request failed - check internet connection",
    );
  }
}
  ...
  try {
    ...
    if (response.ok) {
      ...
      if (response.status === 401) {
        ...
        console.log(
          "   Solution: Get a new key at https://platform.openai.com/api-keys",
        );
      } else if (response.status === 403) {
        console.log(
          "   Issue: Forbidden - billing issue or insufficient permissions",
        );
        console.log(
        ...
        console.log(
          "   Issue: OpenAI server error - their servers are having issues",
        );
